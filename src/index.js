...
;(function () {
  ...
  function findMessagesWrapperStrategy(window) {
    return window.document.querySelector(
      'div[role=grid]  > div > div > div > div, section > div > div > div > div > div > div > div > div[style*=height] > div',
    )
  }
  ...
  function waitFor(target, test, removed = false, timeout = 2000) {
    return new Promise((resolve, reject) => {
      ...
      const treeWalker = target.ownerDocument.createTreeWalker(target, NodeFilter.SHOW_ELEMENT)
  ...
  async function loadMoreMessageStrategy(uiComponent) {
    ...
    try {
      await waitFor(
        uiComponent.root.ownerDocument.body,
        node => {
          if (
            node.nodeType === Node.ELEMENT_NODE &&
            uiComponent.root.contains(node) &&
            uiComponent.root.scrollTop !== 0
          ) {
            return node
          }
  ...
  class UIMessage extends UIComponent {
    ...
    async openActionsMenu() {
      ...
      this.identifier.actionButton = await new Promise((resolve, reject) => {
        setTimeout(() => {
          ...
          reject('ERROR: Unable to find actionButton')
        })
      })
    ...
    async clickUnsend() {
      ...
      this.identifier.dialogButton = await waitFor(this.root.ownerDocument.body, () =>
    ...
    try {
      ...
      for (const item of queue.items.slice()) {
        try {
          await item.promise()
          console.debug(
            `Completed Task ${item.task.id} will continue again in ${window.IDMU_MESSAGE_QUEUE_DELAY}ms`,
          )
    ...
    try {
      const batchSize = parseInt(
        window.prompt(
          ...
          localStorage.getItem('IDMU_BATCH_SIZE') || 5,
        ),
      )
